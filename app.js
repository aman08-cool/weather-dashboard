$(document).ready(function () {
  ...
  function displayCurrentWeather(data) {
    ...
    var tempF = parseInt(data.current.main.temp);
    var tempC = convertTemperature(tempF, true);
    $("#temperature").text(
      isCelsius ? `${tempC.toFixed(1)}\u00B0 C` : `${tempF}\u00B0 F`
    );
  ...
  // Event Listener: Delete search history
  ...
  // Event Listener: Temperature unit toggle
  $(".temperature-toggle").on("click", function () {
    isCelsius = !isCelsius;
    $(this).text(isCelsius ? "°F" : "°C");

    // Update current weather temperature
    var currentTemp = parseFloat($("#temperature").text());
    var convertedTemp = convertTemperature(currentTemp, !isCelsius);
    $("#temperature").text(
      `${convertedTemp.toFixed(1)}\u00B0 ${isCelsius ? "C" : "F"}`
    );

    // Update forecast temperatures
    for (var i = 1; i <= 5; i++) {
      var forecastTemp = parseFloat($(`#day-${i}-temp`).text());
      var convertedForecastTemp = convertTemperature(forecastTemp, !isCelsius);
      $(`#day-${i}-temp`).text(
        `${convertedForecastTemp.toFixed(1)}\u00B0 ${isCelsius ? "C" : "F"}`
      );
    }

    // Update the weather for the current city to reflect the new unit
    if (currentCity) {
